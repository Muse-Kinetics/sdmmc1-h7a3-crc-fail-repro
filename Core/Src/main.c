...
#include "sd_diag.h" /* repro + diagnostics - see sd_diag.c/README.md */
...
int main(void)
{
  ...
  SD_Repro_MountAndListRoot(); // minimal repro - see README.md

  // Diagnostic helpers used while investigating the failure above -
  // uncomment individually as needed. See FINDINGS.md for what each one
  // showed.
  // SD_Diag_DumpCardInfo();       // print CID/CSD and card info
  // SD_Diag_RawBlockRead(8192);   // register-level diag read (STA/DCOUNT) of one block
  // SD_Diag_DmaReadTest(8192);    // IDMA-mode read of one block
  // SD_Diag_DlybSweep();          // sweep the SDMMC1 RX delay-block's sampling phase (~10s)
